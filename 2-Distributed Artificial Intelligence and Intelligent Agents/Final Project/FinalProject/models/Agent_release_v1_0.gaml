...
species requester parent:people {
	...
	{
		do send with: [receivers:: supplies, content:: ['I need supplies.'] ,performative::'request' ,protocol:: 'request_supplies'];
		write 'requester '+string(self) +' sent a message to supply station or deliveryman';
	}
...
species supplies parent:building skills:[communicating]
{
	...
	reflex handle_request when: (!empty(messages)) {
		write 'supply station '+string(self)+' received a message';
	    message requestfromreceiver <- messages at 0;
	    deliveryman best_dm <- deliverymen with_min_of (each.target_cell distance_to requestfromreceiver.sender.location);
	    do reply with: [message::requestfromreceiver, performative::'inform', content::[best_dm]];
	    //remove index:0 from: messages;
	}
