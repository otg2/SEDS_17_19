...
class Triest extends RichMapFunction[(Int,Long,Long), String] {

  private var M:Int = 10
  private var sample: ValueState[ArrayBuffer[Edge]] = _
  private var t: ValueState[Int] = _
  private var tau_u: ValueState[Map[Long, Float]] = _
  private var tau: ValueState[Float] = _
...
object CountTriangles extends App {
  ...
  // For Stream from socket
  //val stream = env.socketTextStream(hostName, port)

  // For Stream from text file

  val stream = env.readTextFile("/Users/mohamedgabr/Desktop/out.ucidata-zachary.csv")

  val result = stream

    .map{_.split(",")}.map(x => (1 , x(0).toLong, x(1).toLong))
