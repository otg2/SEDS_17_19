...
object SentimentAnalysisUtils {
  ...
  def detectSentiment(message: String): List[String] = {
    ...
    var longest = 0
    var mainSentiment = 0
    var weightedSentiment: Double = 0d
    var averageSentiment: Double = 0d
    ...
    if (sentiments.size == 0) {
      mainSentiment = -1
      weightedSentiment = -1d
      averageSentiment = -1d
    }
    else {
      averageSentiment = sentiments.sum / sentiments.size
      val weightedSentiments = (sentiments, sizes).zipped.map((sentiment, size) => sentiment * size)
      weightedSentiment = weightedSentiments.sum / (sizes.fold(0)(_ + _))
    }
    ...
    */
    List(mainSentiment, averageSentiment.round, weightedSentiment.round).map(x => x match {
      case s if s < 0 => NOT_UNDERSTOOD
      case s if s == 0 => VERY_NEGATIVE
      case s if s == 1 => NEGATIVE
      case s if s == 2 => NEUTRAL
      case s if s == 3 => POSITIVE
      case s if s == 4 => VERY_POSITIVE
      case s if s > 4 => NOT_UNDERSTOOD
    }).map(_.toString)
  }
